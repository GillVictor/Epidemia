...
$zoneId = $_GET['id'] ?? null;
if (!$zoneId) {
  header("Location: ../NiceAdmin/elements-zones.php?error=Aucune zone sélectionnée");
  exit;
}
...
// Récupérer le pays de la zone actuelle
$paysAssocie = $zone->getPays();
?>
      ...
      <div class="row justify-content-center">
        <div class="col-lg-10">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">
                <center>Modification de la zone</center>
              </h5>
              ...
              <!-- Formulaire Zone -->
              <form action="../test/modification_zones.php" method="POST">

                <!-- ID de la zone (pour la modification) -->
                <input type="hidden" name="id" value="<?= $zone ? htmlspecialchars($zone->getId()) : '' ?>">
                <!-- Nom de la zone -->
                <div class="row mb-3">
                  <label for="zoneNom" class="col-sm-2 col-form-label">Nom de la Zone</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" id="zoneNom" name="nom" value="<?= htmlspecialchars($zone->getNom()) ?>" required>
                  </div>
                </div>

                <!-- Nombre de cas positifs -->
                <div class="row mb-3">
                  <label for="zonePositifs" class="col-sm-2 col-form-label">Nombre de Cas Positifs</label>
                  <div class="col-sm-10">
                    <input type="number" class="form-control" id="zonePositifs" name="nb_positifs" value="<?= htmlspecialchars($zone->getNbrPositifs()) ?>" required>
                  </div>
                </div>

                <!-- Nombre de cas symptomatiques -->
                <div class="row mb-3">
                  <label for="zoneSymptomatiques" class="col-sm-2 col-form-label">Nombre de Cas Symptomatiques</label>
                  <div class="col-sm-10">
                    <input type="number" class="form-control" id="zoneSymptomatiques" name="nb_symptomatiques" value="<?= htmlspecialchars($zone->getNbrSymptomatiques()) ?>" required>
                  </div>
                </div>

                <!-- Nombre d'habitants -->
                <div class="row mb-3">
                  <label for="zoneHabitants" class="col-sm-2 col-form-label">Nombre d'Habitants</label>
                  <div class="col-sm-10">
                    <input type="number" class="form-control" id="zoneHabitants" name="nb_habitants" value="<?= htmlspecialchars($zone->getNbrHabitants()) ?>" required>
                  </div>
                </div>

                <!-- Choisir le pays -->
                <div class="row mb-3">
                  <label for="zonePays" class="col-sm-2 col-form-label">Choisir ou Ajouter le Pays</label>
                  <div class="col-sm-10">
                    <!-- Liste des pays existants -->
                    <select class="form-control" id="zonePays" name="pays_id">
                      <option value="">-- Sélectionner un pays --</option>
                      <?php foreach ($paysList as $pays) : ?>
                        <option value="<?= htmlspecialchars($pays->getId()) ?>"
                          <?= ($paysAssocie && $paysAssocie->getId() === $pays->getId()) ? 'selected' : '' ?>>
                          <?= htmlspecialchars($pays->getNom()) ?>
                        </option>
                      <?php endforeach; ?>
                    </select>
                    <!-- Option pour saisir un pays -->
                    <input type="text" class="form-control mt-2" id="nouveauPays" name="nouveau_pays" placeholder="Si le pays n'existe pas, saisissez-le ici">
                  </div>
                </div>
                ...
                <div class="row mb-3">
                  <div class="col-sm-10 offset-sm-2">
                    <button type="submit" class="btn btn-success">Modifier la Zone</button>
                  </div>
                </div>
